...
class MultiHeadAttention(nn.Module):
    def __init__(self, size, num_head):
        ...
        self.wq = torch.nn.Parameter(torch.Tensor(size,size))
        self.wk = torch.nn.Parameter(torch.Tensor(size,size))
        self.wv = torch.nn.Parameter(torch.Tensor(size,size))
        self.wo = torch.nn.Parameter(torch.Tensor(size,size))

        self.num_head = num_head

        if size % num_head != 0:
            raise ValueError("size must be dividable by num_head")
    ...
    def forward(self, x):
        ...
        q = torch.cat(q.split(split_size=self.head_size, dim=2), dim=0)
        k = torch.cat(k.split(split_size=self.head_size, dim=2), dim=0)
        v = torch.cat(v.split(split_size=self.head_size, dim=2), dim=0)

        attention = torch.matmul(q, k.transpose(1,2)) / math.sqrt(self.head_size)
        softmax_weight = F.softmax(attention, dim=1)
        weighted_mul = torch.matmul(softmax_weight, v)
...
class GGNN_HAN(nn.Module):
    def __init__(self, network_parameters, device, LOSS_TYPE, tempo_length=1, num_trill_param=5):
        ...
        self.graph_1st = GatedGraph(self.note_hidden_size, self.num_edge_types)
        ...
        self.graph_2nd = GatedGraph(self.note_hidden_size, self.num_edge_types)
...
class GGNN_Recursive(nn.Module):
    def __init__(self, network_parameters, device):
        ...
        self.final_graph = GatedGraph(self.final_input + self.encoder_size + self.output_size, self.num_edge_types, self.output_size)
...
class SGGNN_Alt(nn.Module):
    def __init__(self, network_parameters, device):
        ...
        self.graph_iteration = network_parameters.graph_iteration
        self.num_edge_types = network_parameters.num_edge_types
        self.marginal_entry_size = network_parameters.margin.size
        self.final_graph_input_size = self.final_input + self.encoder_size + self.output_size + self.marginal_entry_size
        ...
        self.beat_tempo_contractor = nn.Sequential(
            nn.Linear(self.final_graph_input_size, self.time_regressive_size),
            nn.Dropout(DROP_OUT),
            nn.ReLU()
        )
        ...
        self.initial_result_fc = nn.Linear(self.final_input, self.output_size)
        self.final_graph = GatedGraph(self.final_graph_input_size, self.num_edge_types, self.device, self.output_size + self.marginal_entry_size)
        self.tempo_rnn = nn.LSTM(self.time_regressive_size + 3+ 5, self.time_regressive_size, num_layers=self.time_regressive_layer, batch_first=True, bidirectional=True)

        self.final_beat_attention = nn.Linear(self.final_graph_input_size, self.final_graph_input_size)
        self.tempo_fc = nn.Linear(self.time_regressive_size * 2, 1)
        # self.fc = nn.Linear(self.final_input + self.encoder_size + self.output_size, self.output_size - 1)
        self.fc = nn.Sequential(
            nn.Linear(self.final_graph_input_size + self.time_regressive_size * 2 + 1 + LEN_DYNAMICS_VEC, self.encoder_size),
            nn.Dropout(DROP_OUT),
            nn.ReLU(),
    ...
    def forward(self, x, y, edges, note_locations, start_index, initial_z=False, return_z=False):
        ...
        measure_perform_style_spanned = self.span_beat_to_note_num(measure_perform_style, measure_numbers, num_notes, start_index)
        initial_margin = torch.zeros((note_out.size(0), num_notes, self.marginal_entry_size)).to(self.device)

        out_with_result = torch.cat((note_out, measure_perform_style_spanned, initial_output, initial_margin), 2)
        ...
        beat_tempo_primo = tempo_primo[0, 0, :].repeat((1, num_beats, 1))
        beat_tempo_vector = self.note_tempo_infos_to_beat(x, beat_numbers, start_index, TEMPO_IDX)
        dynamics_info_vector = x[:, :, DYNAMICS_IDX:DYNAMICS_IDX+LEN_DYNAMICS_VEC]
        ...
        for i in range(5):
            ...
            tempos_spanned = self.span_beat_to_note_num(tempo_out, beat_numbers, num_notes, start_index)
            out_beat_spanned = self.span_beat_to_note_num(out_beat_rnn_result, beat_numbers, num_notes, start_index)
            out_with_beat_out = torch.cat((out_with_result, out_beat_spanned, tempos_spanned, dynamics_info_vector),2)
            other_out = self.fc(out_with_beat_out)

            final_out = torch.cat((tempos_spanned, other_out),2)
            out_with_result = torch.cat((out_with_result[:,:,:-self.output_size-self.marginal_entry_size], final_out, out_with_result[:,:,-self.marginal_entry_size:]),2)
            # out = torch.cat((out, trill_out), 2)
...
class SGGNN_Note(nn.Module):
    def __init__(self, network_parameters, device):
        super(SGGNN_Note, self).__init__()
        self.device = device
        self.input_size = network_parameters.input_size
        self.output_size = network_parameters.output_size
        self.num_layers = network_parameters.note.layer
    ...
    def forward(self, x, y, edges, note_locations, start_index, initial_z=False, return_z=False):
        ...
        note_out, graph_out, measure_hidden_out = self.run_graph_network(x, edges, measure_numbers, start_index)
        if type(initial_z) is not bool:
            if type(initial_z) is list or not initial_z.is_cuda:
                perform_z = torch.Tensor(initial_z).to(self.device).view(1,-1)
            else:
            ...
            perform_concat = torch.cat((note_out, graph_out, y), 2).view(-1, self.encoder_input_size)
            perform_style_contracted = self.performance_contractor(perform_concat).view(1, num_notes, -1)
            perform_style_graphed = self.performance_graph_encoder(perform_style_contracted, edges)
            performance_measure_nodes = self.make_higher_node(perform_style_graphed, self.performance_measure_attention, beat_numbers,
                                                  measure_numbers, start_index, lower_is_note=True)
        ...
        initial_output = self.initial_result_fc(torch.cat((note_out, graph_out), 2))
        num_measures = measure_numbers[start_index+num_notes-1] - measure_numbers[start_index] + 1
        perform_z_measure_spanned = perform_z.repeat(num_measures, 1).view(1,num_measures, -1)
        perform_z_measure_cat = torch.cat((perform_z_measure_spanned, measure_hidden_out), 2)
        measure_perform_style, _ = self.perform_style_to_measure(perform_z_measure_cat)
        ...
        out_with_result = torch.cat((note_out, graph_out, measure_perform_style_spanned, initial_beat_hidden, initial_output, initial_margin), 2)
    ...
    def run_graph_network(self, nodes, adjacency_matrix, measure_numbers, start_index, iteration=5):
        ...
        final_out = torch.cat((notes_hidden, notes_hidden_second),-1)
        return notes_dense_hidden, final_out, measure_hidden
...
class HAN_VAE(nn.Module):
    ...
    def forward(self, x, y, edges, note_locations, start_index, initial_z=False, rand_threshold=0.7, return_z=False):
        beat_numbers = [x.beat for x in note_locations]
        measure_numbers = [x.measure for x in note_locations]
        voice_numbers = [x.voice for x in note_locations]
        onset_numbers = [x.onset for x in note_locations]
...
class BaselineModel(nn.Module):
    def __init__(self, network_parameters, device, num_trill_param=5):
        ...
        self.note_lstm = nn.LSTM(self.note_hidden_size, self.note_hidden_size, self.note_layer_num, batch_first=True, bidirectional=True)


    def forward(self, x, y, edges, note_locations, start_index, initial_z=False, return_z=False):
        note_hidden = self.note_fc(x)
        lstm_result = self.note_lstm()
        return
