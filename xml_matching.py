    ...
    def extract_and_update_score_features(self, piece_data):
        ...
        def _get_beat_importance(piece_data):
            ...
            for i, note in enumerate(piece_data.xml_notes):
                importance = cal_beat_importance(beat_positions[i], note.tempo.time_numerator)
        ...
        def _get_measure_length(piece_data):
            ...
            for _, note in enumerate(piece_data.xml_notes):
                ...
                if measure_index + 1 < len(piece_data.measure_positions):
                    ...
                    measure_length = piece_data.measure_positions[measure_index] - piece_data.measure_positions[measure_index - 1]
                measure_lengthes.append(measure_length / note.state_fixed.divisions)
        ...
        features['midi_pitch'] = [note.pitch[1] for note in piece_data.xml_notes]
        features['pitch'] = [pitch_into_vector(note.pitch[1]) for note in piece_data.xml_notes]
        ...
        features['grace_order'] = [note.note_duration.grace_order for note in piece_data.xml_notes]
        features['is_grace_note'] = [int(note.note_duration.is_grace_note) for note in piece_data.xml_notes]
        features['preceded_by_grace_note'] = [int(note.note_duration.preceded_by_grace_note) for note in piece_data.xml_notes]
        ...
        features['notation'] = [note_notation_to_vector(note) for note in piece_data.xml_notes]
